...
func NewCommandRepository(pool *pgxpool.Pool) pushDomain.CommandRepository {
	return &commandRepository{pool}
}

func (r *commandRepository) Save(ctx context.Context, push pushDomain.Push) error {
	query := `
INSERT INTO pushes (
    phone_number,
    sender,
...
VALUES (
	...
	result, err := r.pool.Exec(ctx, query, push.PhoneNumber, push.Sender, push.Content, push.Status, push.Type, push.ScheduledAt, nil, nil, push.IdempotencyKey, time.Now().Unix(), push.Priority)

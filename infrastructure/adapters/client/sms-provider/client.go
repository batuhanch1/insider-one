...
func (s *smsProvider) Deliver(ctx context.Context, request *DeliverRequest) (*DeliverResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("smsProvider Deliver Marshal error %w", err)
	}

	url := fmt.Sprintf("%s%s", s.config.Host, Path_Deliver)
	req, err := http.NewRequest(http.MethodPost, url, bytes.NewBuffer(bodyBytes))
	if err != nil {
		return nil, fmt.Errorf("smsProvider Deliver NewRequest error %w", err)
	}
	...
	if resp, err = s.client.Do(ctx, req); err != nil {
		return nil, fmt.Errorf("smsProvider Deliver Do error %w", err)
	}
	...
	if bodyByte, err = io.ReadAll(resp.Body); err != nil {
		return nil, fmt.Errorf("smsProvider Deliver ReadAll error %w", err)
	}

	var response DeliverResponse
	if err = json.Unmarshal(bodyByte, &response); err != nil {
		return nil, fmt.Errorf("smsProvider Deliver Unmarshal error %w", err)
	}
	return &response, nil
}
